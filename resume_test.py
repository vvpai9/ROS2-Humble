...
def main():
    ...
    try:
        ...
        node.vehicle.mode = VehicleMode("GUIDED")
        while not node.vehicle.mode == 'GUIDED':
            print("Waiting for mode change...", node.vehicle.mode.name)
            time.sleep(1)
        ...
        node.vehicle.mode = VehicleMode("AUTO")
        while not node.vehicle.mode == 'AUTO':
            print("Waiting for mode change...", node.vehicle.mode.name)
            time.sleep(1)
