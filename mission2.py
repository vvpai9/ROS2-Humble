...
def arm_and_takeoff(aTargetAltitude):
    ...
    while True:
        ...
        if vehicle.location.global_relative_frame.alt >= aTargetAltitude * 0.8:  # Trigger just below target alt.
    ...
    while(1):
        ...
        if vehicle.location.global_frame.alt>10:
            if x != h_x and y != h_y:
            	...
            	send_ned_velocity(0, 0, groundspeed)
            elif y == h_y and x != h_x:
                if x > h_x:
                    ...
                    send_ned_velocity(groundspeed, 0, groundspeed)
                    time.sleep(0.1)
        if vehicle.location.global_relative_frame.alt<=10 and ((x - h_x <= 10 and y - h_y <= 10) or (x - h_x <= -10 and y - h_y <= -10)):
            break
        '''else:
            if x != h_x and y != h_y:
                if x > h_x and y > h_y:
...
def mission():
    ...
    alt = 3
    arm_and_takeoff(alt)
    goto_location_norm(-35.363262, 149.166161, 15)
    landavi()
    time.sleep(4)
